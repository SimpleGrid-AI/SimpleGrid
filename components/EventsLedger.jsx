...
function AbilitySection() {
  const lines = [
    { type: 'prompt',   text: '> Received 200 sheets of 16-gauge steel from Midwest' },
  ...
  return (
    <section className="section" id="ability" style={{ background: '#fff', paddingTop: 140, paddingBottom: 140 }}>
      <div className="container">
        <Reveal>
          <div className="tag">EARLY ADOPTION</div>
          <h2 className="h2 ink" style={{ maxWidth: 1200 }}>
            Floor-ready on day one.<br/>As simple as texting.
          </h2>
          <p className="lead" style={{ maxWidth: '100%' }}>
            Adoption is where ERPs go to die. We removed the menus, the codes, the training manual. Your team types what happened - the system does the rest.
          </p>
        </Reveal>
        ...
        <div style={{ marginTop: 36, display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 36, alignItems: 'center' }}>
          ...
          <Reveal delay={120}>
            <ol className="adopt-flow">
              {[
                { t: 'Type it', p: 'Your team types what happened, in plain language - like a text.' },
                { t: 'Hank reads it', p: 'No SKU codes, no menus, no forms. It understands what you mean.' },
                { t: 'Matched & logged', p: 'Linked to the PO, inventory and your rules - a permanent event with a name and time.' },
                { t: 'Live everywhere', p: 'Floor, office and books update on the spot. Nothing lost in a chat thread.' },
              ].map((x, i) => (
                <li key={i} className="adopt-step">
                  <span className="adopt-node">{i + 1}</span>
                  <div>
                    <div className="adopt-t">{x.t}</div>
                    <p className="adopt-p">{x.p}</p>
                  </div>
                </li>
              ))}
            </ol>
