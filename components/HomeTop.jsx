...
function CycleHeadline() {
  const HEADLINES = [
    <>The Operational Layer<br/>for your floor.</>
  ];
...
function Hero() {
  ...
  // Hero ecosystem: the SimpleGrid live layer, connected down to the accounting
  // software + tools it runs on top of (generalised, not a single vendor).
      ...
      <div className="container">
        <div className="hero-inner">
          <div>
            ...
            <Reveal delay={200}>
              <p className="hero-sub">SimpleGrid is the operations platform that adds a live, intelligent layer over the accounting software you already run. It's the ERP your floor actually uses, configured to your operation and live in 21 days. No migration, no rip-and-replace.</p>
            </Reveal>
            <Reveal delay={400}>
            ...
            <Reveal delay={500}>
              <div className="hero-proof-strip">
                <div className="hps-item"><span className="hps-v">1-3 wks</span><span className="hps-l">to go live</span></div>
                <div className="hps-item"><span className="hps-v">30-day</span><span className="hps-l">live trial</span></div>
                <div className="hps-item"><span className="hps-v">$0</span><span className="hps-l">up front</span></div>
              </div>
            </Reveal>
          </div>
          <Reveal delay={300}>
            <div className="hero-ledger" role="img" aria-label="SimpleGrid captures every floor action live - each with a name and a timestamp - and posts it straight to your books.">
              <div className="hl-head">
                <span className="hl-title">Floor activity</span>
                <span className="hl-live"><span className="hl-live-dot"></span>Live</span>
              </div>
              <div className="hl-feed">
                <div className="hl-track">
                  {[
                    {c:'var(--sg-blue)',src:'Stores',t:'09:42',x:'Received 300 planks · PO 1182'},
                    {c:'var(--sg-green)',src:'QC',t:'09:43',x:'Batch B-77 passed · 580/600'},
                    {c:'var(--sg-purple)',src:'Assembly',t:'09:44',x:'SKU DT-6 → finishing'},
                    {c:'var(--sg-gold)',src:'Dispatch',t:'09:45',x:'Container loaded · 18 CBM'},
                    {c:'var(--sg-blue)',src:'Books',t:'09:45',x:'Invoice posted → QuickBooks'},
                    {c:'var(--sg-blue)',src:'Floor',t:'09:46',x:'Issued 40 hinges to Cell 3'},
                    {c:'var(--sg-green)',src:'Costing',t:'09:47',x:'Job J-204 margin updated'},
                    {c:'var(--sg-blue)',src:'Stores',t:'09:42',x:'Received 300 planks · PO 1182'},
                    {c:'var(--sg-green)',src:'QC',t:'09:43',x:'Batch B-77 passed · 580/600'},
                    {c:'var(--sg-purple)',src:'Assembly',t:'09:44',x:'SKU DT-6 → finishing'},
                    {c:'var(--sg-gold)',src:'Dispatch',t:'09:45',x:'Container loaded · 18 CBM'},
                    ...
                    <div className="hl-row" key={i}>
                      <span className="hl-dot" style={{background:e.c}}></span>
                      <div className="hl-body">
                        <div className="hl-meta"><span className="hl-src">{e.src}</span><span className="hl-time">{e.t}</span></div>
                        <div className="hl-txt">{e.x}</div>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
              <a className="hl-foot" href="syncs.html">Posts to QuickBooks, Tally &amp; 20+ more &rarr;</a>
            </div>
          </Reveal>
        </div>
      </div>
...
function TrustStrip() {
  ...
  return (
    <div aria-label="Recognized by" style={{
      ...
      borderTop:'1px solid #1a1a1a', borderBottom:'1px solid #1a1a1a', background:'var(--bg-alt)'
    }}>
...
function ProblemSection() {
  ...
  const problems = [
    {
      ...
      visual: (
        <svg viewBox="0 0 400 200" style={{width:'100%',height:'auto',display:'block'}} aria-hidden="true">
          {/* Three scattered sources, all funnelling into one person - calm, static. */}
          {[
            { y: 44,  label: 'Spreadsheets' },
            { y: 86,  label: 'WhatsApp' },
            { y: 128, label: 'Phone calls' },
          ].map((s, i) => (
            <g key={i}>
              <path d={`M 140 ${s.y + 15} C 192 ${s.y + 15} 214 100 246 100`} fill="none" stroke="#D8DEE7" strokeWidth="1.5"/>
              <rect x="16" y={s.y} width="124" height="30" rx="8" fill="#F8FAFC" stroke="#E5E7EB"/>
              <text x="78" y={s.y + 19} fontSize="12.5" fill="#475569" fontWeight="600" textAnchor="middle">{s.label}</text>
            </g>
          ))}
          {/* The single point of knowledge - one calm slate node, no glow, no motion. */}
          <circle cx="290" cy="100" r="44" fill="#1F2937"/>
          <circle cx="290" cy="86" r="11" fill="#fff"/>
          <path d="M 272 118 Q 272 98 290 98 Q 308 98 308 118 Z" fill="#fff"/>
          <text x="290" y="168" fontSize="12" fill="#374151" fontWeight="700" textAnchor="middle">One person knows</text>
        </svg>
      )
    },
    {
      n: '02', t: 'Too complex to use', b: 'Built for a thousand businesses, not your floor.',
    ...
    {
      n: '03', t: "Your ERP can't keep up", b: 'Every small change becomes a six-week project.',
  ...
  const go = (d) => { setDir(d); setActive((a) => (a + d + n) % n); };
  ...
  // Short two-line tab labels (verb + descriptor) for the horizontal switcher.
  const tabMeta = [
    { label: 'On memory',   desc: 'spreadsheets + chats' },
    { label: 'Too complex', desc: 'built for the enterprise' },
    { label: 'Too rigid',   desc: 'every change is a project' },
    { label: 'No adoption', desc: "the floor won't log in" },
  ];

  return (
    <section className="section section-dark" id="why-erp" style={{background:'#0B0F17',paddingTop:56,paddingBottom:64}}>
      <div className="container">
        <Reveal>
          <div className="tag">THE VISIBILITY PROBLEM</div>
          <h2 className="h2" style={{color:'#fff'}}>Accounting got the software. Operations got spreadsheets.</h2>
        </Reveal>
        <Reveal delay={100}>
          <div className="vp-tabs" role="tablist" aria-label="Why ERPs fail mid-market">
            {problems.map((x, i) => (
              <button key={i} className={'vp-tab' + (active === i ? ' on' : '')} role="tab" aria-selected={active === i} onClick={() => { setDir(i >= active ? 1 : -1); setActive(i); }}>
                <span className="vp-tab-label">{tabMeta[i].label}</span>
                <span className="vp-tab-desc">{tabMeta[i].desc}</span>
              </button>
            ))}
          </div>
          <div className="vp-stage">
            <button type="button" className="vp-arrow vp-arrow-prev" onClick={() => go(-1)} aria-label="Previous problem">&lsaquo;</button>
            <div className={'vp-card ' + (dir < 0 ? 'vp-prev' : 'vp-next')} key={active} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            <div className="vp-card-head" style={{display:'flex',alignItems:'baseline',gap:12,marginBottom:10}}>
              <div style={{fontFamily:'var(--font-heading)',fontSize:26,fontWeight:700,color:'var(--sg-blue)',letterSpacing:'-0.02em'}}>{cur.n}</div>
              <h3 style={{fontFamily:'var(--font-heading)',fontSize:22,fontWeight:700,color:'var(--fg1)',margin:0,letterSpacing:'-0.01em',lineHeight:1.25}}>{cur.t}</h3>
            </div>
            <p style={{fontSize:15.5,color:'var(--fg1)',lineHeight:1.55,margin:'0 0 16px'}}>{cur.b}</p>
            <div className={'problem-visual' + (cur.isChatDemo ? ' problem-visual-chat' : ' problem-visual-svg')}>
              {cur.isChatDemo ? (
            ...
            {cur.footer && (
              <div style={{marginTop:16,paddingTop:14,borderTop:'1px solid var(--border)',fontFamily:'var(--font-body)',fontSize:15.5,fontWeight:700,color:'var(--sg-blue)',lineHeight:1.4}}>{cur.footer}</div>
            )}
            </div>
            <button type="button" className="vp-arrow vp-arrow-next" onClick={() => go(1)} aria-label="Next problem">&rsaquo;</button>
          </div>
        </Reveal>
      </div>
      <style>{`
