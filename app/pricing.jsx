...
const BUILD_GROUPS = [
  {
    label: 'In the build',
    items: [
      { h: 'Modeling sessions',       p: 'A few calls to map every entity, state, rule, and exception that runs your factory.' },
  ...
  },
  {
    ...
    items: [
      { h: 'Floor-staff training',     p: 'Your team types what happened like on WhatsApp. The training is the conversation.' },
      { h: 'Senior-led onboarding',    p: 'Led by senior engineers. Not a sales engineer, not an account manager.' },
      { h: 'All future rule changes',  p: 'New approval rule, new stage, new QC gate. Configuration, not code. No change orders.' },
    ],
  },
];
...
function PricingPage() {
  ...
  return (<>
    ...
    <section className="section section-dark" style={{paddingTop:88, paddingBottom:64, position:'relative', overflow:'hidden', background:'var(--sg-black)', minHeight:'calc(100vh - 64px)', display:'flex', flexDirection:'column', justifyContent:'center'}}>
      <div className="container" style={{position:'relative', zIndex:2}}>
        ...
        <p className="lead" style={{color:'rgba(255,255,255,0.78)', maxWidth:820, margin:0}}>
          The build is on us, and your books stay in QuickBooks or Tally. Most ERPs charge you to find out if they work - we charge you after you already know.
        </p>
    ...
    <section className="offer section" aria-labelledby="offer-h2">
      <div className="offer-inner">
        <Reveal>
          <p className="offer-eyebrow">The offer</p>
          <h2 className="offer-h2" id="offer-h2">Try it on. Then pay.</h2>
          <p className="offer-sub">Every ERP makes you pay first and hope. We flipped it: we build and run it on your real floor for 30 days, at our cost.</p>
        </Reveal>

        {/* B. three-step timeline */}
        <Reveal delay={100}>
          <ol className="offer-steps">
            <li className="offer-step">
              <div className="offer-step-num">01</div>
              <h3 className="offer-step-h">One intro call, then a quote</h3>
              <p className="offer-step-p">One call, and you get a real number - no drawn-out discovery before you know the price.</p>
            </li>
            <li className="offer-step">
              <div className="offer-step-num">02</div>
              <h3 className="offer-step-h">Then the detailed conversation</h3>
              <p className="offer-step-p">Once the number makes sense, we go deep on your floor - stages, contractors, approvals, costing.</p>
            </li>
            <li className="offer-step">
              <div className="offer-step-num">03</div>
              <h3 className="offer-step-h">A working demo, not a deck</h3>
              <p className="offer-step-p">No scope doc, no slide deck - we build a working demo of your operation and start there.</p>
            </li>
          </ol>
        </Reveal>

        {/* C. two cards as a sequence */}
        <Reveal delay={150}>
          <div className="offer-cards">
            <div className="offer-card offer-card-hero">
              ...
              <p className="offer-card-caption">For the build. For the migration. For the 30 days on your real floor.</p>
              <ul className="offer-checks">
                <li>Configured to your floor.</li>
                <li>Live in 7-21 days, at our cost.</li>
                <li>Run live for 30 days.</li>
                <li>All your data migrated.</li>
            ...
            <div className="offer-card offer-card-outcome">
              ...
              <p className="offer-card-caption">This isn't a discount play. We carried the build, so the price reflects the result, not the risk.</p>
              <ul className="offer-checks offer-checks-muted">
                <li>One monthly subscription - the entire bill.</li>
                <li>Every feature. No tiers, no add-ons, no per-seat fees.</li>
                <li>New features ship regularly, yours automatically.</li>
    ...
    <section className="builds section" aria-labelledby="builds-h2">
      <div className="builds-inner">
        <Reveal>
          <p className="tag">What the build covers</p>
          <h2 className="builds-h2" id="builds-h2">What we build for you before you owe us anything.</h2>
          <p className="builds-sub">All part of the build and the 30-day trial - no add-ons, no professional-services line item, no implementation partner to pay.</p>
        </Reveal>

        {BUILD_GROUPS.map((grp, g) => (
          <div key={g} className="builds-group">
            <Reveal delay={80 + g * 60}>
              <p className="builds-grouplabel">{grp.label}</p>
              <div className="builds-branch" aria-hidden="true">
                <span className="bb-cell"></span>
                <span className="bb-cell"></span>
                <span className="bb-cell"></span>
              </div>
            </Reveal>
            <div className="builds-grid">
              {grp.items.map((it, j) => (
                <Reveal key={j} delay={140 + g * 60 + j * 90}>
                  <div className="builds-card">
                    <span className="builds-index" aria-hidden="true">{String(g * 3 + j + 1).padStart(2, '0')}</span>
                    <h3 className="builds-card-h">{it.h}</h3>
                    <p className="builds-card-p">{it.p}</p>
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        <Reveal delay={240}>
          <p className="builds-allofit">All of it. <span>$0 until it works.</span></p>
        </Reveal>
      </div>
      <style>{`
